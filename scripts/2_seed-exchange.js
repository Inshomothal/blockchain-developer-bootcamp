...
const { random, floor } = require('lodash')
...
async function main() {
    ...
    console.log(`User 1 created as: ${user1.address}\n`)
    const user2 = accounts[1]
    console.log(`User 2 created as: ${user2.address}\n`)
    amount = tokens(10000)
    ...
    for (let k = 1; k <= 3; k++) {
        // User 1 makes order to get tokens
        transaction = await exchange.connect(user1).makeOrder(mETH.address, tokens(floor(random(40) * 5)), DAPP.address, tokens(k*15))
        result = await transaction.wait()
        console.log(`Made order from ${user1.address}\n`)

        
        // User 2 fills orderorderId = result.events[0].args.id
        orderId = result.events[0].args.id
    ...
    // User 1 makes another order to get tokens
